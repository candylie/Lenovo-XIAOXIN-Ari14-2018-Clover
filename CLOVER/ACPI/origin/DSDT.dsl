 ...
 *
 * Disassembly of DSDT.aml, Sun Oct 14 20:26:37 2018
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x000200FC (131324)
 *     Revision         0x02
 *     Checksum         0x2A
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "CB-01   "
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "CB-01   ", 0x00000001)
{
    ...
    External (_SB_.UBTC.ECUC, MethodObj)    // 0 Arguments (from opcode)
    ...
    External (FPSK, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    External (GSMI, UnknownObj)    // (from opcode)
    External (HIGP, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    ...
    External (MPL0, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    ...
    External (PTMC, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    ...
    External (TUSK, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    External (UBTC, DeviceObj)    // (from opcode)
    External (UMAB, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    External (YG53, UnknownObj)    // (from opcode)    // Conflicts with a later declaration

    Name (SS1, Zero)
    Name (SS2, Zero)
    Name (SS3, One)
    Name (SS4, One)
    OperationRegion (GNVS, SystemMemory, 0x6FF68018, 0x0765)
    ...
    OperationRegion (OGNS, SystemMemory, 0x6FF68FD8, 0x00000010)
    ...
    {
        OperationRegion (PDW0, SystemMemory, 0xFDAE04B8, 0x04)
        Field (PDW0, AnyAcc, NoLock, Preserve)
        {
                ,   1, 
            TEMP,   1, 
    ...
    {
        OperationRegion (PDW0, SystemMemory, 0xFDAE0470, 0x04)
        Field (PDW0, AnyAcc, NoLock, Preserve)
        {
            TEMP,   1
        }
    ...
    Scope (_SB.PCI0)
    {
        Device (XHC)
        {
            ...
            PowerResource (PUBS, 0x03, 0x0000)
            {
                ...
                {
                    If (CondRefOf (\_SB.PCI0.LPCB.EC0.ABC5))
                    {
                        Return (0xFF)
                    }
                    Else
                ...
                Method (_ON, 0, NotSerialized)  // _ON_: Power On
                {
                    If (CondRefOf (\_SB.PCI0.LPCB.EC0.ABC5))
                    {
                        Store (Zero, ^^^LPCB.EC0.ABC5)
                    }
                }

                Method (_OFF, 0, NotSerialized)  // _OFF: Power Off
                {
                    If (CondRefOf (\_SB.PCI0.LPCB.EC0.UCHE))
                    {
                        If (LEqual (^^^LPCB.EC0.UCHE, Zero))
                        {
                            If (CondRefOf (\_SB.PCI0.LPCB.EC0.ABC5))
                            {
                                Store (One, ^^^LPCB.EC0.ABC5)
                            }
                        }
                    }
                }
            ...
            Device (RHUB)
            {
                ...
                Device (HSP4)
                {
                    ...
                    Name (PLD4, Package (0x01)
                    {
                        ...
                        {
                            /* 0000 */  0x82, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            /* 0008 */  0x30, 0x0C, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00 
                        }
                    })
                ...
                Device (HSP6)
                {
                    ...
                    Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
                    {
                        If (LAnd (TUSK, LNotEqual (YG53, One)))
                        {
                            Return (NUPC)
                        }
                        Else
                ...
                Device (HSP7)
                {
                    ...
                    Name (_PLD, Package (0x01)  // _PLD: Physical Location of Device
                    {
                        ...
                        {
                            /* 0000 */  0x82, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            /* 0008 */  0x58, 0x1E, 0x80, 0x03, 0x00, 0x00, 0x00, 0x00 
                        }
                    })
                    Method (_S3D, 0, NotSerialized)  // _S3D: S3 Device State
                    {
    ...
    Scope (_SB.PCI0.I2C1)
    {
        Device (TPL1)
        {
            ...
            CreateWordField (SBFB, \_SB.PCI0.I2C1.TPL1._Y1F._ADR, BADR)  // _ADR: Address
            CreateDWordField (SBFB, \_SB.PCI0.I2C1.TPL1._Y1F._SPE, SPED)  // _SPE: Speed
            CreateDWordField (SBFI, \_SB.PCI0.I2C1.TPL1._Y20._INT, INT2)  // _INT: Interrupts
            CreateWordField (SBFG, 0x17, INT1)
            Method (_INI, 0, NotSerialized)  // _INI: Initialize
            {
                If (LLess (OSYS, 0x07DC))
                ...
                Store (GNUM (0x0202000C), INT1)
                Store (INUM (0x0202000C), INT2)
                If (LEqual (SDM1, Zero))
                {
                    SHPO (0x0202000C, One)
                }

                If (LEqual (TPNP, 0x517A))
                {
                    Store ("WCOM517A", _HID)
                    Store (One, HID2)
                    Store (0x0A, BADR)
        ...
        Device (BTH0)
        {
            ...
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                ...
                Name (SBFI, ResourceTemplate ()
                {
                    ...
                    Interrupt (ResourceConsumer, Edge, ActiveLow, ExclusiveAndWake, ,, _Y24)
                    {
                        0x00000000,
                    }
                })
    ...
    Scope (_SB.PCI0.LPCB)
    {
        Device (EC0)
        {
            ...
            {
                Store (Acquire (ECMT, 0x01F4), Local0)
            ...
            Scope (\)
            {
                OperationRegion (LFCN, SystemMemory, 0x6F88D098, 0x00ED)
                    ...
                    If (LEqual (Acquire (HGCT, 0xA000), Zero))
                    {
                        If (LEqual (ECAV, One))
                        {
                            If (LEqual (Acquire (LFCM, 0xA000), Zero))
                            {
                                ...
                                Sleep (0x10)
                                Store (SRAM (0x04, 0xC2), OBRC)
                                Store (B1RC, OBRC)
                                Sleep (0x10)
                                Store (SRAM (0x04, 0xC6), OBPV)
                                Store (B1FV, OBPV)
        ...
        If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
        {
            ...
            If (LEqual (ECON, One))
            {
                If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
                {
                    ...
                    If (LEqual (RECM (0x40), 0x08))
                    {
                        Notify (\_SB.PWRB, 0x02)
                    }

                    If (LEqual (RECM (0x40), 0x05))
                    {
                        Notify (\_SB.LID0, 0x80)
                    }

                    If (LEqual (RECM (0x40), 0x06))
                    {
                        Notify (\_SB.LID0, 0x80)
                    }
                }
    ...
    OperationRegion (MDBG, SystemMemory, 0x6FF55018, 0x00001004)
